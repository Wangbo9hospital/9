# Complete Prospective Data Validation with Trained XGBoost Model
...
# Output directories
output_dir <- "Prospective_Validation_Results_XGBoost"  # Updated directory name
...
# Convert categorical variables to numeric (one-hot encoding simulation)
for(feature in final_features) {
  if(is.factor(prospective_features[[feature]]) || is.character(prospective_features[[feature]])) {
    # Convert factors to numeric codes
    prospective_features[[feature]] <- as.numeric(factor(prospective_features[[feature]]))
  }
}

# Convert to matrix for XGBoost
prospective_matrix <- as.matrix(prospective_features)

cat("Data preprocessing completed. Final feature dimensions:", dim(prospective_matrix), "\n")

# ==================== Model Prediction ====================
cat("Step 3: Making Predictions with XGBoost...\n")

# Unified prediction function for XGBoost
predict_xgboost <- function(model, newdata_matrix) {
...
# Make predictions
pred_results <- predict_xgboost(final_xgb_model, prospective_matrix)
...
if(is.matrix(probabilities)) {
  for(i in 1:ncol(probabilities)) {
    class_name <- colnames(probabilities)[i]
    results[[paste0("Probability_", class_name)]] <- probabilities[, i]
  }
  # Use second class probability for binary classification (assuming positive class is second)
  if(ncol(probabilities) == 2) {
    results$Predicted_Probability <- probabilities[, 2]
  } else {
    results$Predicted_Probability <- apply(probabilities, 1, max)
  }
} else {
  results$Predicted_Probability <- as.numeric(probabilities)
}

cat("XGBoost predictions completed successfully\n")
...
p1 <- ggplot(results, aes(x = Predicted_Diagnosis, fill = Predicted_Diagnosis)) +
  ...
  labs(title = "XGBoost Prediction Distribution - Prospective Data",
...
p2 <- ggplot(results, aes(x = Predicted_Probability)) +
  ...
  labs(title = "XGBoost Prediction Probability Distribution",
...
if(!is.null(roc_obj)) {
  ...
  p4 <- ggplot(roc_data, aes(x = FPR, y = Sensitivity)) +
    ...
    labs(title = paste("XGBoost ROC Curve (AUC =", round(performance_metrics$AUC, 3), ")"),
...
if(has_labels && !all(is.na(prospective_data$Diagnosis)) && length(unique(prospective_data$Diagnosis)) == 2) {
  ...
  p6 <- ggplot(threshold_long, aes(x = Threshold, y = Value, color = Metric)) +
    geom_line(size = 1.2) +
    geom_point(size = 1) +
    labs(title = "XGBoost Performance Metrics vs Decision Threshold",
...
# 7. Feature Importance Visualization (for XGBoost)
if(inherits(final_xgb_model, "xgb.Booster")) {
  # Extract feature importance from XGBoost model
  importance_matrix <- xgb.importance(
    feature_names = colnames(prospective_matrix), 
  ...
  if(!is.null(importance_matrix) && nrow(importance_matrix) > 0) {
    ...
    p7 <- ggplot(top_features, aes(x = reorder(Feature, Gain), y = Gain)) +
  ...
  feature_importance <- sapply(final_features, function(feat) {
    if(is.numeric(prospective_features[[feat]])) {
      var(prospective_features[[feat]], na.rm = TRUE)
    } else {
      length(unique(prospective_features[[feat]]))
    }
  })
  ...
  p7 <- ggplot(feature_imp_df, aes(x = reorder(Feature, Importance), y = Importance)) +
    geom_bar(stat = "identity", fill = "steelblue", alpha = 0.7) +
    coord_flip() +
    labs(title = "Top 15 Features by Variance/Uniqueness",
         x = "Features",
         y = "Importance Score") +
    theme_minimal() +
    theme(plot.title = element_text(hjust = 0.5, size = 14, face = "bold"))
...
ggsave(file.path(plots_dir, "01_XGBoost_Prediction_Distribution.png"), p1, width = 10, height = 6, dpi = 300)
ggsave(file.path(plots_dir, "02_XGBoost_Probability_Distribution.png"), p2, width = 10, height = 6, dpi = 300)
ggsave(file.path(plots_dir, "03_XGBoost_Confusion_Matrix.png"), p3, width = 8, height = 6, dpi = 300)
ggsave(file.path(plots_dir, "04_XGBoost_ROC_Curve.png"), p4, width = 8, height = 6, dpi = 300)
ggsave(file.path(plots_dir, "05_XGBoost_Calibration_Plot.png"), p5, width = 8, height = 6, dpi = 300)
ggsave(file.path(plots_dir, "06_XGBoost_Threshold_Analysis.png"), p6, width = 10, height = 6, dpi = 300)
ggsave(file.path(plots_dir, "07_XGBoost_Feature_Importance.png"), p7, width = 10, height = 8, dpi = 300)
...
saveWorkbook(wb, file.path(output_dir, "Prospective_Validation_Results_XGBoost.xlsx"), overwrite = TRUE)

# Save R objects
saveRDS(results, file.path(output_dir, "prospective_predictions_xgb.rds"))
if(!is.null(performance_metrics)) {
  saveRDS(performance_metrics, file.path(output_dir, "performance_metrics_xgb.rds"))
}
if(!is.null(roc_obj)) {
  saveRDS(roc_obj, file.path(output_dir, "roc_object_xgb.rds"))
}
...
cat("Model type: XGBoost (", class(final_xgb_model)[1], ")\n")
...
cat("\n=== FILES GENERATED ===\n")
cat("- Prospective_Validation_Results_XGBoost.xlsx (Excel report)\n")
cat("- prospective_predictions_xgb.rds (R data)\n")
cat("- 7 XGBoost visualization plots in Plots/ directory\n")

cat("\nFirst few predictions:\n")
print(head(results[, c("SampleID", "Actual_Diagnosis", "Predicted_Diagnosis", "Predicted_Probability")]))

cat("\n=== XGBOOST VALIDATION COMPLETE ===\n")
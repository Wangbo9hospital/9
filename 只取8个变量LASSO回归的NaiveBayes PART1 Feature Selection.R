...
# ==================== Step 3: LASSO Feature Selection (筛选8个变量) ====================
cat("Step 3: LASSO Feature Selection (筛选8个变量)...\n")
...
if(length(selected_features_univariate) < 2) {
  ...
  if(any(is.na(x_lasso)) | any(is.na(y_lasso))) {
    ...
    if(!is.null(cv_fit)) {
      ...
      # 找到选择8个变量的lambda索引
      lambda_idx <- which(num_features_selected == 8)[1]
      
      if(!is.na(lambda_idx)) {
        # 找到选择8个变量的lambda
        lambda_8feat <- lasso_full$lambda[lambda_idx]
        lasso_model_8feat <- glmnet(
          x = x_lasso, 
          y = y_lasso, 
          alpha = 1, 
          lambda = lambda_8feat,
        ...
        lasso_coef_matrix <- as.matrix(coef(lasso_model_8feat))
        ...
        cat("LASSO selected 8 features at lambda =", round(lambda_8feat, 6), "\n")
      } else {
        # 如果没有正好选择8个变量的lambda，找到最接近的
        cat("No lambda selects exactly 8 features, finding closest match...\n")
        # 找到选择变量数量最接近8的lambda
        diff_features <- abs(num_features_selected - 8)
        ...
        cat("LASSO selected", num_selected, "features (closest to 8) at lambda =", round(lambda_closest, 6), "\n")
      }
      
      # 如果LASSO没有选择任何特征，使用单变量分析结果
      if(length(selected_features_lasso) == 0) {
        cat("Warning: LASSO did not select any features, using top 8 univariate features...\n")
        # 取单变量分析中P值最小的8个特征
        if(length(selected_features_univariate) >= 8) {
          selected_features_lasso <- head(selected_features_univariate, 8)
        } else {
      ...
      cat("Using top 8 univariate features due to LASSO failure\n")
      # 取单变量分析中P值最小的8个特征
      if(length(selected_features_univariate) >= 8) {
        final_features <- head(selected_features_univariate, 8)
      } else {
        final_features <- selected_features_univariate
      }
    }
...
# 确保最终选择8个特征（如果可用）
if(length(final_features) > 8) {
  cat("Selected more than 8 features, keeping top 8 by LASSO coefficients...\n")
  # 重新运行LASSO获取系数绝对值
  ...
  # 使用一个较小的lambda以保留更多特征，然后选择系数绝对值最大的8个
  ...
  top8_idx <- order(coef_abs, decreasing = TRUE)[1:min(8, length(coef_abs))]
  final_features <- rownames(lasso_coef_no_intercept)[top8_idx]
} else if(length(final_features) < 8 && length(selected_features_univariate) >= 8) {
  cat("Selected less than 8 features, supplementing with top univariate features...\n")
  # 补充单变量分析中P值最小的特征直到有8个
  top_univariate <- head(selected_features_univariate, 8)
  final_features <- unique(c(final_features, top_univariate))[1:8]
}

cat("Final selected features (target: 8):", length(final_features), "\n")
...
if(length(selected_features_univariate) >= 8) {
  ...
  if(exists("lambda_8feat")) {
    abline(v = log(lambda_8feat), col = "red", lty = 2, lwd = 2)
    legend("topright", legend = paste("8 features (λ =", round(lambda_8feat, 4), ")"), 
...
cat("After LASSO selection (target: 8):", length(final_features), "\n")

cat("\nTop 8 Features by Importance:\n")
for(i in 1:min(8, nrow(top_20_features))) {

# Comprehensive Model Performance Comparison Across All Datasets
...
# ==================== Configuration ====================
cat("=== Comprehensive Model Performance Comparison ===\n")

# File paths
model_path <- "final_gbm_model.rds"  # 修改为GBM模型路径
features_path <- "final_features.rds"
gbm_iter_path <- "gbm_best_iter.rds"
...
# Output directories
output_dir <- "Comprehensive_Model_Performance_GBM"
...
cat("Step 1: Loading Model and Features...\n")

# Load trained GBM model and features
final_gbm_model <- readRDS(model_path)
final_features <- readRDS(features_path)
gbm_best_iter <- readRDS(gbm_iter_path)

cat("GBM Model loaded successfully\n")
...
# ==================== Unified Prediction Function for GBM ====================
predict_gbm <- function(model, newdata, n_trees) {
  # 确保新数据与训练数据具有相同的特征
  if(!all(final_features %in% names(newdata))) {
    missing_features <- setdiff(final_features, names(newdata))
    warning("Missing features in newdata: ", paste(missing_features, collapse = ", "))
  }
  
  # 预测概率
  pred_probs <- predict(model, newdata = newdata, n.trees = n_trees, type = "response")
...
process_dataset <- function(data_path, dataset_name, train_data_ref) {
  ...
  # Make predictions using GBM
  pred_results <- predict_gbm(final_gbm_model, features, gbm_best_iter)
...
cat("Step 2: Processing All Datasets...\n")
...
cat("Step 4: Generating Comprehensive Visualizations...\n")
...
if(nrow(performance_comparison) > 0) {
  ...
  p1 <- ggplot(perf_long, aes(x = Dataset, y = Value, fill = Metric)) +
    ...
    labs(title = "GBM Performance Metrics Comparison Across Datasets",
         x = "Dataset", y = "Value") +
    theme_minimal() +
  ...
  ggsave(file.path(plots_dir, "01_GBM_Performance_Comparison.png"), p1, width = 14, height = 10, dpi = 300)
}
...
if(length(roc_objects) > 0) {
  p2 <- ggplot() +
    labs(title = "GBM ROC Curves Comparison Across Datasets",
  ...
  ggsave(file.path(plots_dir, "02_GBM_ROC_Comparison.png"), p2, width = 10, height = 8, dpi = 300)
}

# 3. Prediction Distribution Across Datasets
p3 <- ggplot(all_results, aes(x = Predicted_Diagnosis, fill = Dataset)) +
  geom_bar(position = "dodge", alpha = 0.7) +
  labs(title = "GBM Prediction Distribution Across Datasets",
       x = "Predicted Diagnosis", y = "Count") +
  theme_minimal() +
  theme(plot.title = element_text(hjust = 0.5, size = 14, face = "bold"),
        legend.position = "top")

ggsave(file.path(plots_dir, "03_GBM_Prediction_Distribution.png"), p3, width = 12, height = 8, dpi = 300)

# 4. Probability Distribution Comparison
p4 <- ggplot(all_results, aes(x = Predicted_Probability, fill = Dataset)) +
  geom_density(alpha = 0.5) +
  labs(title = "GBM Prediction Probability Distribution Across Datasets",
       x = "Predicted Probability", y = "Density") +
  theme_minimal() +
  theme(plot.title = element_text(hjust = 0.5, size = 14, face = "bold"),
        legend.position = "top")

ggsave(file.path(plots_dir, "04_GBM_Probability_Distribution.png"), p4, width = 12, height = 8, dpi = 300)
...
if(length(confusion_matrices) > 0) {
  ...
  p5 <- ggplot(all_cm, aes(x = Actual, y = Predicted, fill = Freq)) +
    ...
    labs(title = "GBM Confusion Matrices Across Datasets",
  ...
  ggsave(file.path(plots_dir, "05_GBM_Confusion_Matrices.png"), p5, width = 12, height = 10, dpi = 300)
}
...
if(nrow(calibration_data) > 0) {
  ...
  p6 <- ggplot(calibration_summary, aes(x = Mean_Predicted, y = Mean_Actual, color = Dataset)) +
    ...
    labs(title = "GBM Calibration Plots Comparison Across Datasets",
  ...
  ggsave(file.path(plots_dir, "06_GBM_Calibration_Comparison.png"), p6, width = 10, height = 8, dpi = 300)
}
...
if(nrow(performance_comparison) >= 3) {
  ...
  p7 <- ggplot(perf_trend, aes(x = Dataset, y = Value, color = Metric, group = Metric)) +
    geom_line(size = 1.2) +
    geom_point(size = 3) +
    labs(title = "GBM Performance Trend Across Datasets",
  ...
  ggsave(file.path(plots_dir, "07_GBM_Performance_Trend.png"), p7, width = 12, height = 8, dpi = 300)
}
...
if(length(final_features) > 0) {
  # Extract relative influence from GBM model
  feature_imp <- summary(final_gbm_model, n.trees = gbm_best_iter, plotit = FALSE)
  
  feature_imp_df <- data.frame(
    Feature = feature_imp$var,
    Relative_Influence = feature_imp$rel.inf
  ) %>%
    arrange(desc(Relative_Influence)) %>%
    head(15)  # Top 15 features
  
  p9 <- ggplot(feature_imp_df, aes(x = reorder(Feature, Relative_Influence), y = Relative_Influence)) +
    geom_bar(stat = "identity", fill = "steelblue", alpha = 0.7) +
    coord_flip() +
    labs(title = "Top 15 Features by Relative Influence (GBM)",
         x = "Features",
         y = "Relative Influence") +
    theme_minimal() +
    theme(plot.title = element_text(hjust = 0.5, size = 14, face = "bold"))
  
  ggsave(file.path(plots_dir, "09_GBM_Feature_Importance.png"), p9, width = 12, height = 8, dpi = 300)
}
...
# 获取GBM模型参数
distribution_name <- if(!is.null(final_gbm_model$distribution)) {
  if(is.character(final_gbm_model$distribution)) {
    final_gbm_model$distribution
  } else if(is.list(final_gbm_model$distribution) && !is.null(final_gbm_model$distribution$name)) {
    final_gbm_model$distribution$name
  } else {
    "Unknown"
  }
} else {
...
model_info <- data.frame(
  Item = c("Model Type", "Number of Features", "Optimal Trees", "Minimum CV Error", 
           "Distribution", "Interaction Depth", "Shrinkage", "Features Used", "Analysis Date"),
  Value = c(
...
if(exists("feature_imp_df")) {
  addWorksheet(wb, "GBM_Feature_Importance")
  feature_imp_full <- summary(final_gbm_model, n.trees = gbm_best_iter, plotit = FALSE)
  feature_imp_full_df <- data.frame(
    Feature = feature_imp_full$var,
    Relative_Influence = feature_imp_full$rel.inf,
    Normalized_Influence = feature_imp_full$rel.inf / sum(feature_imp_full$rel.inf) * 100
  ) %>%
    arrange(desc(Relative_Influence))
  writeData(wb, "GBM_Feature_Importance", feature_imp_full_df, startRow = 1)
}

# Save workbook
saveWorkbook(wb, file.path(output_dir, "GBM_Comprehensive_Performance_Report.xlsx"), overwrite = TRUE)

# Save R objects
saveRDS(all_results, file.path(data_dir, "gbm_all_predictions.rds"))
if(nrow(performance_comparison) > 0) {
  saveRDS(performance_comparison, file.path(data_dir, "gbm_performance_comparison.rds"))
}
saveRDS(dataset_summary, file.path(data_dir, "dataset_summary.rds"))
if(exists("feature_imp_full_df")) {
  saveRDS(feature_imp_full_df, file.path(data_dir, "gbm_feature_importance.rds"))
}

# ==================== Final Summary ====================
cat("\n=== COMPREHENSIVE GBM PERFORMANCE ANALYSIS COMPLETE ===\n")
...
if(nrow(performance_comparison) > 0) {
  ...
  if("Training Set" %in% performance_comparison$Dataset && "Prospective Set" %in% performance_comparison$Dataset) {
    ...
    if(auc_degradation > 0.1) {
      ...
      cat("GOOD: GBM model shows stable performance across datasets\n")
    }
  }
}
...
cat("Optimal number of trees:", gbm_best_iter, "\n")
cat("Minimum CV error:", round(min(final_gbm_model$cv.error), 4), "\n")
cat("Distribution:", distribution_name, "\n")
cat("Interaction depth:", final_gbm_model$interaction.depth, "\n")
cat("Shrinkage parameter:", final_gbm_model$shrinkage, "\n")
cat("Number of features:", length(final_features), "\n")

cat("\n=== FILES GENERATED ===\n")
cat("- GBM_Comprehensive_Performance_Report.xlsx (Excel report)\n")
cat("- 9 comprehensive GBM visualization plots\n")
cat("- R data files for further analysis\n")

cat("\n=== GBM ANALYSIS COMPLETE ===\n")
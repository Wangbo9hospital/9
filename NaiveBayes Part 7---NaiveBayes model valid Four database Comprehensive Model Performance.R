...
final_nb_model <- readRDS(model_path)
final_features <- readRDS(features_path)

cat("Model loaded successfully\n")
cat("Number of features:", length(final_features), "\n")
cat("Model type:", if(!is.null(final_nb_model$model_type)) final_nb_model$model_type else class(final_nb_model)[1], "\n")
...
predict_naive_bayes <- function(model, newdata) {
  ...
  if(model_type == "e1071") {
    pred_probs <- predict(model, newdata = newdata, type = "raw")
    pred_class <- predict(model, newdata = newdata, type = "class")
  } else if(model_type == "klaR") {
    pred_result <- predict(model, newdata = newdata)
    pred_probs <- pred_result$posterior
...
process_dataset <- function(data_path, dataset_name, train_data_ref) {
  ...
  # Make predictions
  pred_results <- predict_naive_bayes(final_nb_model, features)
...
if(nrow(performance_comparison) > 0) {
  ...
  p1 <- ggplot(perf_long, aes(x = Dataset, y = Value, fill = Metric)) +
    ...
    labs(title = "Performance Metrics Comparison Across Datasets",
         x = "Dataset", y = "Value") +
    theme_minimal() +
  ...
  ggsave(file.path(plots_dir, "01_Performance_Comparison.png"), p1, width = 14, height = 10, dpi = 300)
}
...
if(length(roc_objects) > 0) {
  p2 <- ggplot() +
    labs(title = "ROC Curves Comparison Across Datasets",
  ...
  ggsave(file.path(plots_dir, "02_ROC_Comparison.png"), p2, width = 10, height = 8, dpi = 300)
}

# 3. Prediction Distribution Across Datasets
p3 <- ggplot(all_results, aes(x = Predicted_Diagnosis, fill = Dataset)) +
  geom_bar(position = "dodge", alpha = 0.7) +
  labs(title = "Prediction Distribution Across Datasets",
       x = "Predicted Diagnosis", y = "Count") +
  theme_minimal() +
  theme(plot.title = element_text(hjust = 0.5, size = 14, face = "bold"),
        legend.position = "top")

ggsave(file.path(plots_dir, "03_Prediction_Distribution.png"), p3, width = 12, height = 8, dpi = 300)

# 4. Probability Distribution Comparison
p4 <- ggplot(all_results, aes(x = Predicted_Probability, fill = Dataset)) +
  geom_density(alpha = 0.5) +
  labs(title = "Prediction Probability Distribution Across Datasets",
       x = "Predicted Probability", y = "Density") +
  theme_minimal() +
  theme(plot.title = element_text(hjust = 0.5, size = 14, face = "bold"),
        legend.position = "top")

ggsave(file.path(plots_dir, "04_Probability_Distribution.png"), p4, width = 12, height = 8, dpi = 300)
...
if(length(confusion_matrices) > 0) {
  ...
  p5 <- ggplot(all_cm, aes(x = Actual, y = Predicted, fill = Freq)) +
    ...
    labs(title = "Confusion Matrices Across Datasets",
  ...
  ggsave(file.path(plots_dir, "05_Confusion_Matrices.png"), p5, width = 12, height = 10, dpi = 300)
}
...
if(nrow(calibration_data) > 0) {
  ...
  p6 <- ggplot(calibration_summary, aes(x = Mean_Predicted, y = Mean_Actual, color = Dataset)) +
    ...
    labs(title = "Calibration Plots Comparison Across Datasets",
  ...
  ggsave(file.path(plots_dir, "06_Calibration_Comparison.png"), p6, width = 10, height = 8, dpi = 300)
}
...
if(nrow(performance_comparison) >= 3) {
  ...
  p7 <- ggplot(perf_trend, aes(x = Dataset, y = Value, color = Metric, group = Metric)) +
    geom_line(size = 1.2) +
    geom_point(size = 3) +
    labs(title = "Performance Trend Across Datasets",
  ...
  ggsave(file.path(plots_dir, "07_Performance_Trend.png"), p7, width = 12, height = 8, dpi = 300)
}
...
model_info <- data.frame(
  ...
  Value = c(
    if(!is.null(final_nb_model$model_type)) final_nb_model$model_type else class(final_nb_model)[1],
    length(final_features),
    paste(final_features, collapse = ", "),
    if(!is.null(final_nb_model$training_date)) as.character(final_nb_model$training_date) else "Unknown",
...
# Save workbook
saveWorkbook(wb, file.path(output_dir, "Comprehensive_Performance_Report.xlsx"), overwrite = TRUE)

# Save R objects
saveRDS(all_results, file.path(data_dir, "all_predictions.rds"))
if(nrow(performance_comparison) > 0) {
  saveRDS(performance_comparison, file.path(data_dir, "performance_comparison.rds"))
}
...
if(nrow(performance_comparison) > 0) {
  ...
  if("Training Set" %in% performance_comparison$Dataset && "Prospective Set" %in% performance_comparison$Dataset) {
    ...
    if(auc_degradation > 0.1) {
      ...
      cat("GOOD: Model shows stable performance across datasets\n")
    }
  }
}

cat("\n=== FILES GENERATED ===\n")
cat("- Comprehensive_Performance_Report.xlsx (Excel report)\n")
cat("- 8 comprehensive visualization plots\n")

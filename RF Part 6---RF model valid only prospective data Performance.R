# Complete Prospective Data Validation with Trained Random Forest Model
...
# Output directories
output_dir <- "Prospective_Validation_Results_RF"
...
# ==================== Model Prediction ====================
cat("Step 3: Making Predictions with Random Forest...\n")
...
# Make predictions
pred_results <- predict_random_forest(final_rf_model, prospective_features)
...
if(is.matrix(probabilities)) {
  ...
  # Use second class probability for binary classification
...
cat("RF Predictions completed successfully\n")
...
p1 <- ggplot(results, aes(x = Predicted_Diagnosis, fill = Predicted_Diagnosis)) +
  ...
  labs(title = "RF Prediction Distribution - Prospective Data",
...
p2 <- ggplot(results, aes(x = Predicted_Probability)) +
  ...
  labs(title = "RF Prediction Probability Distribution",
...
if(!is.null(confusion_matrix)) {
  ...
  p3 <- ggplot(cm_data, aes(x = Reference, y = Prediction, fill = Freq)) +
    ...
    labs(title = "RF Confusion Matrix Heatmap",
         x = "Actual Diagnosis",
...
if(!is.null(roc_obj)) {
  ...
  p4 <- ggplot(roc_data, aes(x = FPR, y = Sensitivity)) +
    ...
    labs(title = paste("RF ROC Curve (AUC =", round(performance_metrics$AUC, 3), ")"),
...
if(has_labels && !all(is.na(prospective_data$Diagnosis)) && length(unique(prospective_data$Diagnosis)) == 2) {
  ...
  p5 <- ggplot(calibration_summary, aes(x = Mean_Predicted, y = Mean_Actual)) +
    ...
    labs(title = "RF Calibration Plot",
...
if(has_labels && !all(is.na(prospective_data$Diagnosis)) && length(unique(prospective_data$Diagnosis)) == 2) {
  ...
  p6 <- ggplot(threshold_long, aes(x = Threshold, y = Value, color = Metric)) +
    geom_line(size = 1.2) +
    geom_point(size = 1) +
    labs(title = "RF Performance Metrics vs Decision Threshold",
...
# 7. Feature Importance Visualization (for Random Forest)
if(!is.null(final_rf_model$importance) && length(final_features) > 0) {
  ...
  if(!is.null(final_rf_model$importance)) {
    ...
    imp_scores <- sapply(final_features, function(feat) {
...
ggsave(file.path(plots_dir, "01_RF_Prediction_Distribution.png"), p1, width = 10, height = 6, dpi = 300)
ggsave(file.path(plots_dir, "02_RF_Probability_Distribution.png"), p2, width = 10, height = 6, dpi = 300)
ggsave(file.path(plots_dir, "03_RF_Confusion_Matrix.png"), p3, width = 8, height = 6, dpi = 300)
ggsave(file.path(plots_dir, "04_RF_ROC_Curve.png"), p4, width = 8, height = 6, dpi = 300)
ggsave(file.path(plots_dir, "05_RF_Calibration_Plot.png"), p5, width = 8, height = 6, dpi = 300)
ggsave(file.path(plots_dir, "06_RF_Threshold_Analysis.png"), p6, width = 10, height = 6, dpi = 300)
ggsave(file.path(plots_dir, "07_RF_Feature_Importance.png"), p7, width = 10, height = 8, dpi = 300)
...
# Predictions sheet
addWorksheet(wb, "RF_Predictions")
writeData(wb, "RF_Predictions", results, startRow = 1)

# Performance metrics sheet
if(!is.null(performance_metrics)) {
  addWorksheet(wb, "RF_Performance_Metrics")
  metrics_df <- data.frame(
    Metric = names(performance_metrics)[1:8],
    Value = unlist(performance_metrics[1:8])
  )
  writeData(wb, "RF_Performance_Metrics", metrics_df, startRow = 1)
  
  # Confusion matrix sheet
  addWorksheet(wb, "RF_Confusion_Matrix")
  cm_df <- as.data.frame(performance_metrics$Confusion_Matrix)
  writeData(wb, "RF_Confusion_Matrix", cm_df, startRow = 1)
}

# Data summary sheet
addWorksheet(wb, "RF_Data_Summary")
...
writeData(wb, "RF_Data_Summary", summary_df, startRow = 1)

# Save threshold analysis if available
if(exists("threshold_analysis")) {
  addWorksheet(wb, "RF_Threshold_Analysis")
  writeData(wb, "RF_Threshold_Analysis", threshold_analysis, startRow = 1)
}
...
saveWorkbook(wb, file.path(output_dir, "RF_Prospective_Validation_Results.xlsx"), overwrite = TRUE)

# Save R objects
saveRDS(results, file.path(output_dir, "rf_prospective_predictions.rds"))
if(!is.null(performance_metrics)) {
  saveRDS(performance_metrics, file.path(output_dir, "rf_performance_metrics.rds"))
}
if(!is.null(roc_obj)) {
  saveRDS(roc_obj, file.path(output_dir, "rf_roc_object.rds"))
}
...
cat("\n=== FILES GENERATED ===\n")
cat("- RF_Prospective_Validation_Results.xlsx (Excel report)\n")
cat("- rf_prospective_predictions.rds (R data)\n")
cat("- 7 RF visualization plots in Plots/ directory\n")

cat("\nFirst few predictions:\n")
print(head(results[, c("SampleID", "Actual_Diagnosis", "Predicted_Diagnosis", "Predicted_Probability")]))

cat("\n=== RF VALIDATION COMPLETE ===\n")
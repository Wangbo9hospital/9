# Complete Prospective Data Validation with Trained NaiveBayes Model
...
# Make predictions
pred_results <- predict_naive_bayes(final_nb_model, prospective_features)
...
p1 <- ggplot(results, aes(x = Predicted_Diagnosis, fill = Predicted_Diagnosis)) +
  ...
  labs(title = "Prediction Distribution - Prospective Data",
...
if(!is.null(roc_obj)) {
  ...
  p4 <- ggplot(roc_data, aes(x = FPR, y = Sensitivity)) +
    ...
    labs(title = paste("ROC Curve (AUC =", round(performance_metrics$AUC, 3), ")"),
...
if(!is.null(final_nb_model$tables) && length(final_features) > 0) {

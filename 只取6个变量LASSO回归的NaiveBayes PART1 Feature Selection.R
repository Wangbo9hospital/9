...
# ==================== Step 3: LASSO Feature Selection (筛选6个变量) ====================
cat("Step 3: LASSO Feature Selection (筛选6个变量)...\n")
...
if(length(selected_features_univariate) < 2) {
  ...
  if(any(is.na(x_lasso)) | any(is.na(y_lasso))) {
    ...
    if(!is.null(cv_fit)) {
      ...
      # 找到选择6个变量的lambda索引
      lambda_idx <- which(num_features_selected == 6)[1]
      
      if(!is.na(lambda_idx)) {
        # 找到选择6个变量的lambda
        lambda_6feat <- lasso_full$lambda[lambda_idx]
        lasso_model_6feat <- glmnet(
          x = x_lasso, 
          y = y_lasso, 
          alpha = 1, 
          lambda = lambda_6feat,
        ...
        lasso_coef_matrix <- as.matrix(coef(lasso_model_6feat))
        ...
        cat("LASSO selected 6 features at lambda =", round(lambda_6feat, 6), "\n")
      } else {
        # 如果没有正好选择6个变量的lambda，找到最接近的
        cat("No lambda selects exactly 6 features, finding closest match...\n")
        # 找到选择变量数量最接近6的lambda
        diff_features <- abs(num_features_selected - 6)
        ...
        cat("LASSO selected", num_selected, "features (closest to 6) at lambda =", round(lambda_closest, 6), "\n")
      }
      
      # 如果LASSO没有选择任何特征，使用单变量分析结果
      if(length(selected_features_lasso) == 0) {
        cat("Warning: LASSO did not select any features, using top 6 univariate features...\n")
        # 取单变量分析中P值最小的6个特征
        if(length(selected_features_univariate) >= 6) {
          selected_features_lasso <- head(selected_features_univariate, 6)
        } else {
      ...
      cat("Using top 6 univariate features due to LASSO failure\n")
      # 取单变量分析中P值最小的6个特征
      if(length(selected_features_univariate) >= 6) {
        final_features <- head(selected_features_univariate, 6)
      } else {
        final_features <- selected_features_univariate
      }
    }
...
if(length(final_features) > 6) {
  cat("Selected more than 6 features, keeping top 6 by LASSO coefficients...\n")
  # 重新运行LASSO获取系数绝对值
  ...
  # 使用一个较小的lambda以保留更多特征，然后选择系数绝对值最大的6个
  ...
  top6_idx <- order(coef_abs, decreasing = TRUE)[1:min(6, length(coef_abs))]
  final_features <- rownames(lasso_coef_no_intercept)[top6_idx]
} else if(length(final_features) < 6 && length(selected_features_univariate) >= 6) {
  cat("Selected less than 6 features, supplementing with top univariate features...\n")
  # 补充单变量分析中P值最小的特征直到有6个
  top_univariate <- head(selected_features_univariate, 6)
  final_features <- unique(c(final_features, top_univariate))[1:6]
}

cat("Final selected features (target: 6):", length(final_features), "\n")
...
if(length(selected_features_univariate) >= 6) {
  ...
  if(exists("lambda_6feat")) {
    abline(v = log(lambda_6feat), col = "red", lty = 2, lwd = 2)
    legend("topright", legend = paste("6 features (λ =", round(lambda_6feat, 4), ")"), 
...
cat("After LASSO selection (target: 6):", length(final_features), "\n")

cat("\nTop 6 Features by Importance:\n")
for(i in 1:min(6, nrow(top_20_features))) {

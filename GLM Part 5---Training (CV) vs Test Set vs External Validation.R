...
final_glm_model <- readRDS("final_glm_model.rds")  # 修改为GLM模型
...
cat("=== Comparative Analysis: Training (CV) vs Test Set vs External Set Performance (GLM) ===\n")
...
# 重新计算测试集预测概率以确保一致性 - 使用GLM预测函数
test_probs_recalc <- predict(final_glm_model, newdata = test_features_subset, type = "response")
...
p_bar <- ggplot(comparison_long, aes(x = Metric, y = Value, fill = Dataset)) +
  ...
  labs(title = "Performance Metrics Comparison: Training (CV) vs Test vs External (GLM)",
       y = "Score", x = "Metric") +
...
p_prob <- ggplot(prob_comparison, aes(x = Probability, fill = Dataset)) +
  ...
  labs(title = "Predicted Probability Distribution by Class and Dataset (GLM)",
...
p_cal <- ggplot(calibration_combined, aes(x = Mean_Predicted, y = Mean_Actual, color = Dataset)) +
  ...
  labs(title = "Calibration Curve Comparison: Training (CV) vs Test vs External (GLM)",
...
p_degradation <- ggplot(degradation_long, aes(x = Comparison, y = Metric, fill = Drop)) +
  ...
  labs(title = "Performance Degradation Analysis Across Datasets (GLM)",
...
comprehensive_plot <- (p_bar + p_cal) / (p_prob + p_degradation) +
  plot_annotation(
    title = "Comprehensive Model Performance: Training (CV) vs Test vs External Sets (GLM)",

...
# ==================== Step 3: LASSO Feature Selection (筛选5个变量) ====================
cat("Step 3: LASSO Feature Selection (筛选5个变量)...\n")
...
if(length(selected_features_univariate) < 2) {
  ...
  if(any(is.na(x_lasso)) | any(is.na(y_lasso))) {
    ...
    if(!is.null(cv_fit)) {
      ...
      # 找到选择5个变量的lambda索引
      lambda_idx <- which(num_features_selected == 5)[1]
      
      if(!is.na(lambda_idx)) {
        # 找到选择5个变量的lambda
        lambda_5feat <- lasso_full$lambda[lambda_idx]
        lasso_model_5feat <- glmnet(
          x = x_lasso, 
          y = y_lasso, 
          alpha = 1, 
          lambda = lambda_5feat,
        ...
        lasso_coef_matrix <- as.matrix(coef(lasso_model_5feat))
        ...
        cat("LASSO selected 5 features at lambda =", round(lambda_5feat, 6), "\n")
      } else {
        # 如果没有正好选择5个变量的lambda，找到最接近的
        cat("No lambda selects exactly 5 features, finding closest match...\n")
        # 找到选择变量数量最接近5的lambda
        diff_features <- abs(num_features_selected - 5)
        ...
        cat("LASSO selected", num_selected, "features (closest to 5) at lambda =", round(lambda_closest, 6), "\n")
      }
      
      # 如果LASSO没有选择任何特征，使用单变量分析结果
      if(length(selected_features_lasso) == 0) {
        cat("Warning: LASSO did not select any features, using top 5 univariate features...\n")
        # 取单变量分析中P值最小的5个特征
        if(length(selected_features_univariate) >= 5) {
          selected_features_lasso <- head(selected_features_univariate, 5)
        } else {
      ...
      cat("Using top 5 univariate features due to LASSO failure\n")
      # 取单变量分析中P值最小的5个特征
      if(length(selected_features_univariate) >= 5) {
        final_features <- head(selected_features_univariate, 5)
      } else {
        final_features <- selected_features_univariate
      }
    }
...
# 确保最终选择5个特征（如果可用）
if(length(final_features) > 5) {
  cat("Selected more than 5 features, keeping top 5 by LASSO coefficients...\n")
  # 重新运行LASSO获取系数绝对值
  ...
  # 使用一个较小的lambda以保留更多特征，然后选择系数绝对值最大的5个
  ...
  top5_idx <- order(coef_abs, decreasing = TRUE)[1:min(5, length(coef_abs))]
  final_features <- rownames(lasso_coef_no_intercept)[top5_idx]
} else if(length(final_features) < 5 && length(selected_features_univariate) >= 5) {
  cat("Selected less than 5 features, supplementing with top univariate features...\n")
  # 补充单变量分析中P值最小的特征直到有5个
  top_univariate <- head(selected_features_univariate, 5)
  final_features <- unique(c(final_features, top_univariate))[1:5]
}

cat("Final selected features (target: 5):", length(final_features), "\n")
...
# Save top 5 features
top_5_features <- head(feature_importance_df, 5)

# ==================== Step 7: Generate Visualizations ====================
cat("Step 7: Generating Visualizations...\n")
if(!dir.exists("Part1_Results")) dir.create("Part1_Results")

# 1. Top 5 Feature Importance Plot
if(nrow(top_5_features) > 0) {
  p1 <- ggplot(top_5_features, aes(x = reorder(Feature, Importance), y = Importance)) +
    geom_bar(stat = "identity", fill = "steelblue", alpha = 0.8) +
    coord_flip() +
    labs(title = "Top 5 Feature Importance - NaiveBayes Model",
         x = "Features",
         y = "Importance Score") +
    theme_minimal() +
  ...
  ggsave("Part1_Results/Top5_Feature_Importance.png", p1, width = 12, height = 8, dpi = 300)
}
...
if(length(selected_features_univariate) >= 5) {
  ...
  abline(v = log(lasso_plot_model$lambda[which.min(abs(lasso_plot_model$lambda - lambda_5feat))]), 
         col = "red", lty = 2, lwd = 2)
  dev.off()
}
...
cat("After LASSO selection (target: 5):", length(final_features), "\n")

...
cat("=== Part 2: 10-Fold Cross-Validation with RF Imputation (GBM) ===\n")
...
for(i in 1:10) {
  ...
  # 为GBM准备数据 - 创建数值型诊断变量
  fold_train_balanced$Diagnosis_numeric <- as.numeric(fold_train_balanced$Diagnosis) - 1
  fold_test_imputed$Diagnosis_numeric <- as.numeric(fold_test_imputed$Diagnosis) - 1
  
  # Train GBM Model
  cat("  Training GBM model for fold", i, "...\n")
  
  tryCatch({
    fold_model <- gbm(
      formula = Diagnosis_numeric ~ .,
      data = fold_train_balanced[, c(available_features_balanced, "Diagnosis_numeric")],
    ...
    # 为当前fold选择最优树数量
    fold_best_iter <- gbm.perf(fold_model, method = "cv", plot.it = FALSE)
    cat("  GBM model trained successfully for fold", i, "- Optimal trees:", fold_best_iter, "\n")
    
  }, error = function(e) {
    cat("  Failed to train GBM model for fold", i, ":", e$message, "\n")
    # 如果训练失败，跳过这个fold
  ...
  if(!exists("fold_model") || is.null(fold_model)) {
    cat("  GBM model is NULL for fold", i, ", skipping...\n")
    next
  }
  ...
  tryCatch({
    # 使用GBM进行预测
    fold_probs <- predict(fold_model, 
                          newdata = fold_test_imputed, 
                          n.trees = fold_best_iter, 
                          type = "response")
...
p1 <- ggplot(cv_long, aes(x = Metric, y = Value, fill = Metric)) +
  ...
  labs(title = "10-Fold Cross-Validation Performance Metrics - GBM",
...
if(length(cv_rocs) > 0 && any(!sapply(cv_rocs, is.null))) {
  ...
  plot(1, type = "n", xlim = c(0, 1), ylim = c(0, 1), 
       ...
       main = "10-Fold Cross-Validation ROC Curves - GBM",
...
p2 <- ggplot(cv_results, aes(x = AUC)) +
  ...
  labs(title = "Distribution of AUC across 10-Fold CV - GBM",
       x = "AUC",
       y = "Frequency") +
  theme_minimal() +
  theme(plot.title = element_text(hjust = 0.5, size = 14, face = "bold"))
...
if(length(misclassified_cases) > 0 && nrow(do.call(rbind, misclassified_cases)) > 0) {
  ...
  p3 <- ggplot(all_misclassified, aes(x = as.factor(True_Label), fill = as.factor(Predicted_Label))) +
    ...
    labs(title = "Misclassification Analysis - GBM",
         x = "True Label",
         y = "Count",
...
if(nrow(cv_predictions) > 0) {
  p4 <- ggplot(cv_predictions, aes(x = Predicted_Probability, fill = as.factor(True_Label))) +
    ...
    labs(title = "Distribution of CV Predicted Probabilities by True Label - GBM",
  ...
  p5 <- ggplot(cv_predictions, aes(x = as.factor(Fold), y = Predicted_Probability, fill = as.factor(True_Label))) +
    geom_boxplot(alpha = 0.7) +
    labs(title = "Predicted Probabilities Distribution by Fold - GBM",
         x = "Fold",
...
cat("GBM Parameters:\n")
cat("  Distribution:", gbm_params$distribution, "\n")
cat("  Interaction depth:", gbm_params$interaction.depth, "\n")
cat("  Shrinkage (learning rate):", gbm_params$shrinkage, "\n")
cat("  Minimum observations in node:", gbm_params$n.minobsinnode, "\n")
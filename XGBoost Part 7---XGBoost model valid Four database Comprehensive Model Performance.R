# Comprehensive Model Performance Comparison Across All Datasets with XGBoost
...
# ==================== Configuration ====================
cat("=== Comprehensive Model Performance Comparison with XGBoost===\n")

# File paths
model_path <- "final_xgb_model.rds"  # Changed to XGBoost model
...
# Output directories
output_dir <- "Comprehensive_Model_Performance_XGBoost"  # Updated directory name
...
cat("Step 1: Loading XGBoost Model and Features...\n")
...
cat("Model type:", class(final_xgb_model)[1], "\n")

# ==================== Unified Prediction Function for XGBoost ====================
predict_xgboost <- function(model, newdata) {
  
  # Convert categorical variables to numeric
  for(feature in colnames(newdata)) {
    if(is.factor(newdata[[feature]]) || is.character(newdata[[feature]])) {
      newdata[[feature]] <- as.numeric(factor(newdata[[feature]]))
    }
  }
  ...
  if(inherits(model, "xgb.Booster")) {
    ...
    pred_probs <- predict(model, newdata = newdata_matrix, reshape = TRUE)
    
    # If binary classification, probabilities are for positive class
    if(length(pred_probs) == nrow(newdata_matrix)) {
      # Binary classification
      pred_probs_matrix <- cbind(1 - pred_probs, pred_probs)
      colnames(pred_probs_matrix) <- c("0", "1")
      pred_class <- ifelse(pred_probs > 0.5, "1", "0")
    } else {
      # Multi-class classification
      pred_probs_matrix <- matrix(pred_probs, nrow = nrow(newdata_matrix), byrow = TRUE)
      class_names <- 0:(ncol(pred_probs_matrix)-1)
      colnames(pred_probs_matrix) <- as.character(class_names)
      pred_class <- as.character(apply(pred_probs_matrix, 1, which.max) - 1)
    }
    
  } else if(inherits(model, "train")) {
    # Caret-trained model
    pred_probs <- predict(model, newdata = as.data.frame(newdata_matrix), type = "prob")
    pred_class <- predict(model, newdata = as.data.frame(newdata_matrix))
    pred_probs_matrix <- as.matrix(pred_probs)
    
  } else {
    stop("Unsupported XGBoost model type")
  }
  
  return(list(probabilities = pred_probs_matrix, predictions = pred_class))
}
...
process_dataset <- function(data_path, dataset_name, train_data_ref) {
  ...
  for(feature in final_features) {
    if(is.factor(features[[feature]]) || is.character(features[[feature]])) {
      features[[feature]] <- as.numeric(factor(features[[feature]]))
    }
  }
  ...
  pred_results <- predict_xgboost(final_xgb_model, features)
...
cat("Step 2: Processing All Datasets with XGBoost...\n")
...
cat("Step 3: Performance Comparison...\n")
...
cat("Step 4: Generating Comprehensive Visualizations for XGBoost...\n")
...
if(nrow(performance_comparison) > 0) {
  ...
  p1 <- ggplot(perf_long, aes(x = Dataset, y = Value, fill = Metric)) +
    ...
    labs(title = "XGBoost Performance Metrics Comparison Across Datasets",
         x = "Dataset", y = "Value") +
    theme_minimal() +
  ...
  ggsave(file.path(plots_dir, "01_XGBoost_Performance_Comparison.png"), p1, width = 14, height = 10, dpi = 300)
}
...
if(length(roc_objects) > 0) {
  p2 <- ggplot() +
    labs(title = "XGBoost ROC Curves Comparison Across Datasets",
  ...
  ggsave(file.path(plots_dir, "02_XGBoost_ROC_Comparison.png"), p2, width = 10, height = 8, dpi = 300)
}

# 3. Prediction Distribution Across Datasets
p3 <- ggplot(all_results, aes(x = Predicted_Diagnosis, fill = Dataset)) +
  geom_bar(position = "dodge", alpha = 0.7) +
  labs(title = "XGBoost Prediction Distribution Across Datasets",
       x = "Predicted Diagnosis", y = "Count") +
  theme_minimal() +
  theme(plot.title = element_text(hjust = 0.5, size = 14, face = "bold"),
        legend.position = "top")

ggsave(file.path(plots_dir, "03_XGBoost_Prediction_Distribution.png"), p3, width = 12, height = 8, dpi = 300)

# 4. Probability Distribution Comparison
p4 <- ggplot(all_results, aes(x = Predicted_Probability, fill = Dataset)) +
  geom_density(alpha = 0.5) +
  labs(title = "XGBoost Prediction Probability Distribution Across Datasets",
       x = "Predicted Probability", y = "Density") +
  theme_minimal() +
  theme(plot.title = element_text(hjust = 0.5, size = 14, face = "bold"),
        legend.position = "top")

ggsave(file.path(plots_dir, "04_XGBoost_Probability_Distribution.png"), p4, width = 12, height = 8, dpi = 300)
...
if(length(confusion_matrices) > 0) {
  ...
  p5 <- ggplot(all_cm, aes(x = Actual, y = Predicted, fill = Freq)) +
    ...
    labs(title = "XGBoost Confusion Matrices Across Datasets",
  ...
  ggsave(file.path(plots_dir, "05_XGBoost_Confusion_Matrices.png"), p5, width = 12, height = 10, dpi = 300)
}
...
if(nrow(calibration_data) > 0) {
  ...
  p6 <- ggplot(calibration_summary, aes(x = Mean_Predicted, y = Mean_Actual, color = Dataset)) +
    ...
    labs(title = "XGBoost Calibration Plots Comparison Across Datasets",
  ...
  ggsave(file.path(plots_dir, "06_XGBoost_Calibration_Comparison.png"), p6, width = 10, height = 8, dpi = 300)
}
...
if(nrow(performance_comparison) >= 3) {
  ...
  p7 <- ggplot(perf_trend, aes(x = Dataset, y = Value, color = Metric, group = Metric)) +
    geom_line(size = 1.2) +
    geom_point(size = 3) +
    labs(title = "XGBoost Performance Trend Across Datasets",
  ...
  ggsave(file.path(plots_dir, "07_XGBoost_Performance_Trend.png"), p7, width = 12, height = 8, dpi = 300)
}
...
p8 <- ggplot(dataset_summary, aes(x = Dataset, y = Total_Samples, fill = Dataset)) +
  ...
  labs(title = "Sample Size Distribution Across Datasets (XGBoost)",
...
ggsave(file.path(plots_dir, "08_XGBoost_Sample_Distribution.png"), p8, width = 10, height = 8, dpi = 300)

# 9. XGBoost Feature Importance (if available)
if(inherits(final_xgb_model, "xgb.Booster")) {
  ...
  if(!is.null(importance_matrix) && nrow(importance_matrix) > 0) {
    # Get top 15 features
    top_features <- importance_matrix[1:min(15, nrow(importance_matrix)), ]
    
    p9 <- ggplot(top_features, aes(x = reorder(Feature, Gain), y = Gain)) +
      geom_bar(stat = "identity", fill = "steelblue", alpha = 0.7) +
      coord_flip() +
      labs(title = "XGBoost Top 15 Features by Importance",
           x = "Features",
           y = "Gain") +
      theme_minimal() +
      theme(plot.title = element_text(hjust = 0.5, size = 14, face = "bold"))
    
    ggsave(file.path(plots_dir, "09_XGBoost_Feature_Importance.png"), p9, width = 10, height = 8, dpi = 300)
  }
} else if(inherits(final_xgb_model, "train") && !is.null(final_xgb_model$finalModel$feature_names)) {
  # For caret-trained XGBoost models
  importance_matrix <- xgb.importance(
    feature_names = final_xgb_model$finalModel$feature_names, 
    model = final_xgb_model$finalModel
  )
...
cat("Step 5: Generating Comprehensive XGBoost Report...\n")
...
if(nrow(performance_comparison) > 0) {
  writeData(wb, "Performance_Comparison", performance_comparison, startRow = 1)
}

# 2. All Predictions Sheet
addWorksheet(wb, "All_Predictions")
writeData(wb, "All_Predictions", all_results, startRow = 1)

# 3. Dataset Summary Sheet
addWorksheet(wb, "Dataset_Summary")
writeData(wb, "Dataset_Summary", dataset_summary, startRow = 1)
...
model_info <- data.frame(
  ...
  Value = c(
    paste("XGBoost (", class(final_xgb_model)[1], ")", sep = ""),
    length(final_features),
    paste(final_features, collapse = ", "),
    if(!is.null(final_xgb_model$training_date)) as.character(final_xgb_model$training_date) else "Unknown",
    as.character(Sys.Date())
  )
)
writeData(wb, "Model_Information", model_info, startRow = 1)

# 5. Confusion Matrices Sheet
if(length(confusion_matrices) > 0) {
  addWorksheet(wb, "Confusion_Matrices")
  all_confusion <- do.call(rbind, confusion_matrices)
  writeData(wb, "Confusion_Matrices", all_confusion, startRow = 1)
}

# 6. Feature Importance Sheet (if available)
if(exists("importance_matrix") && !is.null(importance_matrix)) {
  addWorksheet(wb, "Feature_Importance")
  writeData(wb, "Feature_Importance", importance_matrix, startRow = 1)
}

# Save workbook
saveWorkbook(wb, file.path(output_dir, "Comprehensive_XGBoost_Performance_Report.xlsx"), overwrite = TRUE)

# Save R objects
saveRDS(all_results, file.path(data_dir, "all_xgb_predictions.rds"))
if(nrow(performance_comparison) > 0) {
  saveRDS(performance_comparison, file.path(data_dir, "xgb_performance_comparison.rds"))
}
saveRDS(dataset_summary, file.path(data_dir, "xgb_dataset_summary.rds"))

# ==================== Final Summary ====================
cat("\n=== COMPREHENSIVE XGBOOST PERFORMANCE ANALYSIS COMPLETE ===\n")
...
if(nrow(performance_comparison) > 0) {
  ...
  if("Training Set" %in% performance_comparison$Dataset && "Prospective Set" %in% performance_comparison$Dataset) {
    ...
    if(auc_degradation > 0.1) {
      ...
      cat("GOOD: XGBoost model shows stable performance across datasets\n")
    }
  }
}

cat("\n=== FILES GENERATED ===\n")
cat("- Comprehensive_XGBoost_Performance_Report.xlsx (Excel report)\n")
cat("- 9 comprehensive XGBoost visualization plots\n")

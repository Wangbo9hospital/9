...
for(i in 1:10) {
  ...
  cat("  Training Random Forest model for fold", i, "...\n")
  set.seed(123 + i)
  ...
  fold_model <- randomForest(
    ...
    importance = FALSE,  # 为了加速训练，不计算重要性
    do.trace = FALSE,
    strata = fold_train_labels,
    sampsize = if(class_ratio_fold < 0.3) {
      rep(min(table(fold_train_labels)), nlevels(fold_train_labels))
    } else {
      nrow(fold_train_features)
    }
  )
  
  # Predict and evaluate
  fold_probs <- predict(fold_model, fold_test_features, type = "prob")[, 2]
...
p1 <- ggplot(cv_long, aes(x = Metric, y = Value, fill = Metric)) +
  ...
  labs(title = "10-Fold Cross-Validation Performance Metrics - Random Forest",
...
plot(1, type = "n", xlim = c(0, 1), ylim = c(0, 1), 
     ...
     main = "10-Fold Cross-Validation ROC Curves - Random Forest",
...
p2 <- ggplot(cv_results, aes(x = AUC)) +
  ...
  labs(title = "Distribution of AUC across 10-Fold CV - Random Forest",
       x = "AUC",
       y = "Frequency") +
  theme_minimal() +
  theme(plot.title = element_text(hjust = 0.5, size = 14, face = "bold"))
...
if(length(misclassified_cases) > 0) {
  ...
  p3 <- ggplot(all_misclassified, aes(x = as.factor(True_Label), fill = as.factor(Predicted_Label))) +
    ...
    labs(title = "Misclassification Analysis - Random Forest",
         x = "True Label",
         y = "Count",
...
p4 <- ggplot(cv_predictions, aes(x = Predicted_Probability, fill = as.factor(True_Label))) +
  ...
  labs(title = "Distribution of CV Predicted Probabilities by True Label - Random Forest",
...
p5 <- ggplot(cv_predictions, aes(x = as.factor(Fold), y = Predicted_Probability, fill = as.factor(True_Label))) +
  geom_boxplot(alpha = 0.7) +
  labs(title = "Predicted Probabilities Distribution by Fold - Random Forest",
       x = "Fold",

...
# ==================== Step 3: LASSO Feature Selection (使用lambda.1se) ====================
cat("Step 3: LASSO Feature Selection (使用lambda.1se)...\n")
...
if(length(selected_features_univariate) < 2) {
  ...
  if(any(is.na(x_lasso)) | any(is.na(y_lasso))) {
    ...
    if(!is.null(cv_fit)) {
      # 使用lambda.1se（1个标准误差内的最简约lambda）作为筛选标准
      ...
      cat("LASSO selected", length(selected_features_lasso), "features at lambda.1se =", round(lambda_1se, 6), "\n")
    } else {
...
if(exists("cv_fit") && !is.null(cv_fit)) {
  ...
  abline(v = log(cv_fit$lambda.1se), col = "red", lty = 2, lwd = 2)
  legend("topright", legend = paste("lambda.1se (λ =", round(cv_fit$lambda.1se, 4), ")"), 
...
cat("After LASSO selection (使用lambda.1se):", length(final_features), "\n")

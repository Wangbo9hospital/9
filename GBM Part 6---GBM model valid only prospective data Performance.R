# Complete Prospective Data Validation with Trained GBM Model
...
# File paths
model_path <- "final_gbm_model.rds"
...
# Output directories
output_dir <- "Prospective_Validation_Results_GBM"
...
# ==================== Model Prediction ====================
cat("Step 3: Making Predictions with GBM...\n")

# GBM prediction function
predict_gbm <- function(model, newdata, n_trees) {
  # Get probabilities
  probabilities <- predict(model, newdata = newdata, n.trees = n_trees, type = "response")
...
# Make predictions
pred_results <- predict_gbm(final_gbm_model, prospective_features, gbm_best_iter)
...
cat("GBM Predictions completed successfully\n")
...
p1 <- ggplot(results, aes(x = Predicted_Diagnosis, fill = Predicted_Diagnosis)) +
  ...
  labs(title = "Prediction Distribution - Prospective Data (GBM)",
...
p2 <- ggplot(results, aes(x = Predicted_Probability)) +
  ...
  labs(title = "Prediction Probability Distribution (GBM)",
...
if(!is.null(confusion_matrix)) {
  ...
  p3 <- ggplot(cm_data, aes(x = Reference, y = Prediction, fill = Freq)) +
    ...
    labs(title = "Confusion Matrix Heatmap (GBM)",
         x = "Actual Diagnosis",
...
if(!is.null(roc_obj)) {
  ...
  p4 <- ggplot(roc_data, aes(x = FPR, y = Sensitivity)) +
    ...
    labs(title = paste("ROC Curve (AUC =", round(performance_metrics$AUC, 3), ") - GBM"),
...
if(has_labels && !all(is.na(prospective_data$Diagnosis)) && length(unique(prospective_data$Diagnosis)) == 2) {
  ...
  p5 <- ggplot(calibration_summary, aes(x = Mean_Predicted, y = Mean_Actual)) +
    ...
    labs(title = "Calibration Plot (GBM)",
...
if(has_labels && !all(is.na(prospective_data$Diagnosis)) && length(unique(prospective_data$Diagnosis)) == 2) {
  ...
  p6 <- ggplot(threshold_long, aes(x = Threshold, y = Value, color = Metric)) +
    geom_line(size = 1.2) +
    geom_point(size = 1) +
    labs(title = "Performance Metrics vs Decision Threshold (GBM)",
...
# 7. Feature Importance Visualization (GBM relative influence)
if(length(final_features) > 0) {
  ...
  p7 <- ggplot(feature_imp_df, aes(x = reorder(Feature, Relative_Influence), y = Relative_Influence)) +
...
summary_df <- data.frame(
  Item = c("Prospective Samples", "Features Used", "Has Ground Truth", 
           "Model Type", "Optimal Trees", "Minimum CV Error", "Prediction Date"),
...
# GBM model feature importance sheet
if(length(final_features) > 0) {
  ...
  feature_imp_df <- data.frame(
    Feature = feature_imp$var,
    Relative_Influence = feature_imp$rel.inf,
    Normalized_Influence = feature_imp$rel.inf / sum(feature_imp$rel.inf) * 100
  ) %>%
    arrange(desc(Relative_Influence))
  writeData(wb, "GBM_Feature_Importance", feature_imp_df, startRow = 1)
}
...
# GBM model statistics sheet - 使用完全手动控制的行数
addWorksheet(wb, "GBM_Model_Statistics")
...
# 手动填充每一行
model_stats[1, ] <- c("Optimal Trees", as.character(gbm_best_iter))
model_stats[2, ] <- c("CV Error", as.character(round(min(final_gbm_model$cv.error), 4)))
model_stats[3, ] <- c("Training Samples", as.character(final_gbm_model$n.train))
model_stats[4, ] <- c("Features", as.character(length(final_features)))
model_stats[5, ] <- c("Interaction Depth", as.character(final_gbm_model$interaction.depth))

writeData(wb, "GBM_Model_Statistics", model_stats, startRow = 1)

saveWorkbook(wb, file.path(output_dir, "Prospective_Validation_Results_GBM.xlsx"), overwrite = TRUE)

# Save R objects
saveRDS(results, file.path(output_dir, "prospective_predictions_gbm.rds"))
if(!is.null(performance_metrics)) {
  saveRDS(performance_metrics, file.path(output_dir, "performance_metrics_gbm.rds"))
}
if(!is.null(roc_obj)) {
  saveRDS(roc_obj, file.path(output_dir, "roc_object_gbm.rds"))
}
...
cat("Optimal number of trees:", gbm_best_iter, "\n")
cat("Minimum CV error:", round(min(final_gbm_model$cv.error), 4), "\n")
cat("Training samples:", final_gbm_model$n.train, "\n")
cat("Number of features:", length(final_features), "\n")
cat("Interaction depth:", final_gbm_model$interaction.depth, "\n")
cat("Shrinkage parameter:", final_gbm_model$shrinkage, "\n")

cat("\n=== FILES GENERATED ===\n")
cat("- Prospective_Validation_Results_GBM.xlsx (Excel report)\n")
cat("- prospective_predictions_gbm.rds (R data)\n")

...
final_nb_model <- readRDS("final_nb_model.rds")  # 修改为NaiveBayes模型
...
cat("=== Comparative Analysis: Training (CV) vs Test Set vs External Set Performance (NaiveBayes) ===\n")
...
# 准备测试数据 - NaiveBayes不需要转换为矩阵
test_features_subset <- test_features_all[, final_features, drop = FALSE]
test_labels <- as.numeric(test_data$Diagnosis) - 1

# 重新计算测试集预测概率以确保一致性 - 使用NaiveBayes预测函数
test_probs_recalc <- predict_naive_bayes(final_nb_model, test_features_subset)
...
# 绘制ROC曲线，使用legacy.axes=TRUE确保从0开始
plot(train_cv_roc, col = "blue", lwd = 3, lty = 1, 
     main = "ROC Curves: Train vs Internal vs External Sets (NaiveBayes)",
     xlab = "False positive rate", ylab = "True positive rate", 
     cex.lab = 1.3, cex.axis = 1.2, cex.main = 1.5,
     legacy.axes = TRUE)  # 确保从0开始

# 添加其他ROC曲线
lines(test_roc, col = "red", lwd = 3, lty = 1)
lines(external_roc, col = "darkgreen", lwd = 3, lty = 1)
...
legend("bottomright", 
       legend = c(
         paste("Train (AUC = ", round(train_cv_auc, 3), " [", 
               round(train_cv_auc_ci[1], 3), "-", round(train_cv_auc_ci[3], 3), "])", sep = ""),
         paste("Internal (AUC = ", round(test_auc, 3), " [", 
               round(test_auc_ci[1], 3), "-", round(test_auc_ci[3], 3), "])", sep = ""),
         paste("External (AUC = ", round(external_auc, 3), " [", 
               round(external_auc_ci[1], 3), "-", round(external_auc_ci[3], 3), "])", sep = "")
       ),
       col = c("blue", "red", "darkgreen"), lwd = 3, lty = 1, cex = 1.2,
       bg = "white", box.lwd = 1)
...
p_bar <- ggplot(comparison_long, aes(x = Metric, y = Value, fill = Dataset)) +
  ...
  labs(title = "Performance Metrics Comparison: Training (CV) vs Test vs External (NaiveBayes)",
       y = "Score", x = "Metric") +
...
p_prob <- ggplot(prob_comparison, aes(x = Probability, fill = Dataset)) +
  ...
  labs(title = "Predicted Probability Distribution by Class and Dataset (NaiveBayes)",
...
p_cal <- ggplot(calibration_combined, aes(x = Mean_Predicted, y = Mean_Actual, color = Dataset)) +
  ...
  labs(title = "Calibration Curve Comparison: Training (CV) vs Test vs External (NaiveBayes)",
...
p_degradation <- ggplot(degradation_long, aes(x = Comparison, y = Metric, fill = Drop)) +
  ...
  labs(title = "Performance Degradation Analysis Across Datasets (NaiveBayes)",
...
comprehensive_plot <- (p_bar + p_cal) / (p_prob + p_degradation) +
  plot_annotation(
    title = "Comprehensive Model Performance: Training (CV) vs Test vs External Sets (NaiveBayes)",

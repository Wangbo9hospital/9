...
# File paths
model_path <- "final_glm_model.rds"  # 修改为GLM模型路径
...
# Output directories
output_dir <- "Comprehensive_Model_Performance"
...
cat("Model type:", class(final_glm_model)[1], "\n")

# ==================== Unified Prediction Function for GLM ====================
predict_glm <- function(model, newdata) {
  # 确保新数据与训练数据具有相同的特征
  if(!all(model$coefnames %in% names(newdata))) {
    missing_features <- setdiff(model$coefnames, names(newdata))
...
process_dataset <- function(data_path, dataset_name, train_data_ref) {
  ...
  pred_results <- predict_glm(final_glm_model, features)
...
if(nrow(performance_comparison) > 0) {
  ...
  p1 <- ggplot(perf_long, aes(x = Dataset, y = Value, fill = Metric)) +
    ...
    labs(title = "GLM Performance Metrics Comparison Across Datasets",
         x = "Dataset", y = "Value") +
    theme_minimal() +
  ...
  ggsave(file.path(plots_dir, "01_GLM_Performance_Comparison.png"), p1, width = 14, height = 10, dpi = 300)
}
...
if(length(roc_objects) > 0) {
  p2 <- ggplot() +
    labs(title = "GLM ROC Curves Comparison Across Datasets",
  ...
  ggsave(file.path(plots_dir, "02_GLM_ROC_Comparison.png"), p2, width = 10, height = 8, dpi = 300)
}

# 3. Prediction Distribution Across Datasets
p3 <- ggplot(all_results, aes(x = Predicted_Diagnosis, fill = Dataset)) +
  geom_bar(position = "dodge", alpha = 0.7) +
  labs(title = "GLM Prediction Distribution Across Datasets",
       x = "Predicted Diagnosis", y = "Count") +
  theme_minimal() +
  theme(plot.title = element_text(hjust = 0.5, size = 14, face = "bold"),
        legend.position = "top")

ggsave(file.path(plots_dir, "03_GLM_Prediction_Distribution.png"), p3, width = 12, height = 8, dpi = 300)

# 4. Probability Distribution Comparison
p4 <- ggplot(all_results, aes(x = Predicted_Probability, fill = Dataset)) +
  geom_density(alpha = 0.5) +
  labs(title = "GLM Prediction Probability Distribution Across Datasets",
       x = "Predicted Probability", y = "Density") +
  theme_minimal() +
  theme(plot.title = element_text(hjust = 0.5, size = 14, face = "bold"),
        legend.position = "top")

ggsave(file.path(plots_dir, "04_GLM_Probability_Distribution.png"), p4, width = 12, height = 8, dpi = 300)
...
if(length(confusion_matrices) > 0) {
  ...
  p5 <- ggplot(all_cm, aes(x = Actual, y = Predicted, fill = Freq)) +
    ...
    labs(title = "GLM Confusion Matrices Across Datasets",
  ...
  ggsave(file.path(plots_dir, "05_GLM_Confusion_Matrices.png"), p5, width = 12, height = 10, dpi = 300)
}
...
if(nrow(calibration_data) > 0) {
  ...
  p6 <- ggplot(calibration_summary, aes(x = Mean_Predicted, y = Mean_Actual, color = Dataset)) +
    ...
    labs(title = "GLM Calibration Plots Comparison Across Datasets",
  ...
  ggsave(file.path(plots_dir, "06_GLM_Calibration_Comparison.png"), p6, width = 10, height = 8, dpi = 300)
}
...
if(nrow(performance_comparison) >= 3) {
  ...
  p7 <- ggplot(perf_trend, aes(x = Dataset, y = Value, color = Metric, group = Metric)) +
    geom_line(size = 1.2) +
    geom_point(size = 3) +
    labs(title = "GLM Performance Trend Across Datasets",
  ...
  ggsave(file.path(plots_dir, "07_GLM_Performance_Trend.png"), p7, width = 12, height = 8, dpi = 300)
}
...
model_info <- data.frame(
  ...
  Value = c(
    ...
    if(!is.null(final_glm_model$training_date)) as.character(final_glm_model$training_date) else "Unknown",
...
# Save workbook
saveWorkbook(wb, file.path(output_dir, "GLM_Comprehensive_Performance_Report.xlsx"), overwrite = TRUE)

# Save R objects
saveRDS(all_results, file.path(data_dir, "glm_all_predictions.rds"))
if(nrow(performance_comparison) > 0) {
  saveRDS(performance_comparison, file.path(data_dir, "glm_performance_comparison.rds"))
}
saveRDS(dataset_summary, file.path(data_dir, "dataset_summary.rds"))

# ==================== Final Summary ====================
cat("\n=== COMPREHENSIVE GLM PERFORMANCE ANALYSIS COMPLETE ===\n")
...
if(nrow(performance_comparison) > 0) {
  ...
  if("Training Set" %in% performance_comparison$Dataset && "Prospective Set" %in% performance_comparison$Dataset) {
    ...
    if(auc_degradation > 0.1) {
      ...
      cat("GOOD: GLM model shows stable performance across datasets\n")
    }
  }
}

cat("\n=== FILES GENERATED ===\n")
cat("- GLM_Comprehensive_Performance_Report.xlsx (Excel report)\n")
cat("- 8 comprehensive GLM visualization plots\n")

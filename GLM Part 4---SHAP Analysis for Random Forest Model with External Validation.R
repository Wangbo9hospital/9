# Part 4: Model Interpretation for GLM Model (with External Validation)
...
cat("=== Part 4: GLM Model Interpretation (Test + External) ===\n")
...
# 方法1: 基于系数的重要性 (Coefficient Importance)
calculate_coefficient_importance <- function(glm_model) {
  # 提取系数（排除截距）
...
calculate_permutation_importance <- function(model, features, labels, n_permutations = 10) {
  # 首先计算基准性能
  baseline_probs <- predict(model, newdata = features, type = "response")
  ...
  for(i in 1:ncol(features)) {
    ...
    for(j in 1:n_permutations) {
      ...
      # 使用置换后的数据进行预测
      perm_probs <- predict(model, newdata = permuted_features, type = "response")
...
# 计算系数重要性
coef_importance <- calculate_coefficient_importance(final_glm_model)

# 计算测试集置换重要性
cat("Calculating permutation importance for test set...\n")
perm_importance_test <- calculate_permutation_importance(final_glm_model, test_features, test_labels, n_permutations = 5)

# 计算外部数据集置换重要性
cat("Calculating permutation importance for external set...\n")
perm_importance_external <- calculate_permutation_importance(final_glm_model, external_features, external_labels, n_permutations = 5)
...
feature_importance_test <- data.frame(
  ...
  Coefficient_Importance = as.numeric(coef_importance[names(perm_importance_test)]),
  Dataset = "Test",
  stringsAsFactors = FALSE
)
feature_importance_test$Combined_Importance <- (feature_importance_test$Permutation_Importance + 
                                                  feature_importance_test$Coefficient_Importance) / 2
...
feature_importance_external <- data.frame(
  ...
  Coefficient_Importance = as.numeric(coef_importance[names(perm_importance_external)]),
  Dataset = "External",
  stringsAsFactors = FALSE
)
feature_importance_external$Combined_Importance <- (feature_importance_external$Permutation_Importance + 
                                                      feature_importance_external$Coefficient_Importance) / 2
...
importance_comparison <- merge(
  feature_importance_test[, c("Feature", "Combined_Importance", "Permutation_Importance", "Coefficient_Importance")],
  feature_importance_external[, c("Feature", "Combined_Importance", "Permutation_Importance", "Coefficient_Importance")],
  by = "Feature",
...
p1 <- ggplot(importance_comparison_long, aes(x = reorder(Feature, Importance), y = Importance, fill = Dataset)) +
  ...
  coord_flip() +
  labs(title = "Feature Importance Comparison - GLM Model",
       subtitle = "Combined Importance (Coefficient + Permutation)",
       x = "Features",
...
p2 <- ggplot(importance_comparison, aes(x = Combined_Importance_Test, y = Combined_Importance_External)) +
  ...
  geom_text(aes(label = ifelse(Rank_Difference > 5 | Difference > 0.05, Feature, "")), 
            vjust = -0.5, size = 3) +
  labs(title = "Feature Importance Correlation: Test vs External Dataset - GLM",
...
create_partial_dependence <- function(model, features, feature_name, grid_size = 20) {
  ...
  for(i in 1:length(feature_range)) {
    ...
    # 预测
    pred_probs <- predict(model, newdata = modified_features, type = "response")
...
for(feature in top_10_features_combined) {
  if(feature %in% colnames(train_features)) {
    # 测试集部分依赖
    pd_data_test <- create_partial_dependence(final_glm_model, test_features, feature)
    pd_data_test$Dataset <- "Test"
    
    # 外部集部分依赖
    pd_data_external <- create_partial_dependence(final_glm_model, external_features, feature)
...
for(i in seq_along(test_sample_indices)) {
  ...
  # 获取预测概率
  pred_prob <- predict(final_glm_model, newdata = case_features, type = "response")
  ...
  for(feature in final_features) {
    if(feature %in% names(coefficients)) {
      # 计算特征对线性预测的贡献
      feature_value <- as.numeric(case_features[[feature]])
      feature_effect <- coefficients[feature] * feature_value
      feature_effects[feature] <- feature_effect
    }
  }
...
p_stability <- ggplot(importance_comparison, aes(x = Rank_Difference, y = Difference)) +
  ...
  geom_text(aes(label = ifelse(Difference > 0.05 | Rank_Difference > 10, Feature, "")), 
            vjust = -0.5, size = 3) +
  labs(title = "Model Stability Analysis: Feature Importance Consistency - GLM",
...
for(feature in top_10_features_combined) {
  if(feature %in% colnames(train_features)) {
    # 测试集部分依赖
    pd_data_test <- create_partial_dependence(final_glm_model, test_features, feature, grid_size = 10)
    pd_data_test$Feature_Name <- feature
    pd_data_test$Dataset <- "Test"
    pd_summary_test <- rbind(pd_summary_test, pd_data_test)
    
    # 外部集部分依赖
    pd_data_external <- create_partial_dependence(final_glm_model, external_features, feature, grid_size = 10)
...
model_coefficients <- data.frame(
  Feature = names(coef(final_glm_model)),
  Coefficient = as.numeric(coef(final_glm_model)),
  Odds_Ratio = exp(as.numeric(coef(final_glm_model))),
  stringsAsFactors = FALSE
)
model_coefficients <- model_coefficients[order(-abs(model_coefficients$Coefficient)), ]

writeData(wb4, "Model_Coefficients", model_coefficients, startRow = 1)
...
interpretation_summary <- data.frame(
  ...
  Description = c(
    ...
    "Pseudo R-squared (1 - residual/null deviance)"
  )
)
...
saveRDS(model_coefficients, "model_coefficients.rds")
...
for(i in 1:min(10, nrow(feature_importance_test))) {
  cat(i, ".", feature_importance_test$Feature[i], 
      ...
      "Coef:", round(feature_importance_test$Coefficient_Importance[i], 4), ")\n")
}
...
for(i in 1:min(10, nrow(feature_importance_external))) {
  cat(i, ".", feature_importance_external$Feature[i], 
      ...
      "Coef:", round(feature_importance_external$Coefficient_Importance[i], 4), ")\n")
}
...
cat("- Coefficient-based and permutation-based feature importance for both datasets\n")
...
cat("- GLM coefficients and odds ratios\n")

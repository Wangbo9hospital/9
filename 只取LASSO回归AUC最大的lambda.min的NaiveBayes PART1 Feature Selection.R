...
# ==================== Step 3: LASSO Feature Selection (使用lambda.min) ====================
cat("Step 3: LASSO Feature Selection (使用lambda.min)...\n")
...
if(length(selected_features_univariate) < 2) {
  ...
  if(any(is.na(x_lasso)) | any(is.na(y_lasso))) {
    ...
    if(!is.null(cv_fit)) {
      # 使用lambda.min（交叉验证中使AUC最大的lambda）而不是lambda.1se
      lambda_min <- cv_fit$lambda.min
      lasso_model_min <- glmnet(
        x = x_lasso, 
        y = y_lasso, 
        alpha = 1, 
        lambda = lambda_min,
      ...
      lasso_coef_matrix <- as.matrix(coef(lasso_model_min))
      ...
      cat("LASSO selected", length(selected_features_lasso), "features at lambda.min =", round(lambda_min, 6), "\n")
    } else {
...
# 4. LASSO交叉验证图
if(exists("cv_fit") && !is.null(cv_fit)) {
  # 保存LASSO交叉验证图
  png("Part1_Results/LASSO_Cross_Validation.png", width = 10, height = 8, units = "in", res = 300)
  plot(cv_fit, main = "LASSO Cross-Validation")
  abline(v = log(cv_fit$lambda.min), col = "red", lty = 2, lwd = 2)
  legend("topright", legend = paste("lambda.min (λ =", round(cv_fit$lambda.min, 4), ")"), 
...
cat("After LASSO selection (使用lambda.min):", length(final_features), "\n")

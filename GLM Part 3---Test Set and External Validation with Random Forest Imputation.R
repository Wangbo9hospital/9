...
cat("Model class:", class(final_glm_model), "\n")
...
cat("Number of coefficients:", length(coef(final_glm_model)), "\n")
cat("\nModel summary:\n")
print(summary(final_glm_model))
cat("========================================\n\n")
...
# ==================== Test Set Validation ====================
cat("=== Part 3: Test Set Validation with RF Imputation (GLM) ===\n")
...
# 使用GLM进行预测
cat("Predicting with GLM model...\n")
tryCatch({
  test_probs_final <- predict(final_glm_model, newdata = test_features_subset, type = "response")
  cat("GLM prediction successful!\n")
  cat("Prediction probabilities range:", range(test_probs_final), "\n")
}, error = function(e) {
  cat("GLM prediction failed:", e$message, "\n")
  # 备选方案：使用默认概率
...
# Predict on external dataset using GLM
cat("Predicting on external dataset with GLM...\n")
tryCatch({
  external_probs_final <- predict(final_glm_model, newdata = external_features_subset, type = "response")
  cat("External dataset GLM prediction successful!\n")
}, error = function(e) {
  cat("External dataset GLM prediction failed:", e$message, "\n")
...
plot(test_roc, main = "ROC Curves Comparison - GLM", 
...
p3 <- ggplot(comparison_data, aes(x = Metric, y = Value, fill = Dataset)) +
  ...
  labs(title = "Performance Comparison: Cross-Validation vs Test Set vs External Set (GLM)",
...
p4 <- ggplot(prob_comparison, aes(x = Probability, fill = as.factor(True_Label))) +
  ...
  labs(title = "Probability Distribution Comparison (GLM)",
...
create_calibration_plot <- function(probabilities, true_labels, dataset_name) {
  ...
  p <- ggplot(calibration_summary, aes(x = Mean_Predicted, y = Mean_Actual)) +
    ...
    labs(title = paste("Calibration Plot -", dataset_name, "(GLM)"),
...
if(exists("misclassified_test_cases") && exists("misclassified_ext_cases")) {
  ...
  p5 <- ggplot(all_misclassified, aes(x = as.factor(True_Label), fill = as.factor(Predicted_Label))) +
    ...
    labs(title = "Misclassification Analysis Comparison (GLM)",
         x = "True Label",
         y = "Count",

...
# ==================== Step 3: LASSO Feature Selection (筛选10个变量) ====================
cat("Step 3: LASSO Feature Selection (筛选10个变量)...\n")
...
if(length(selected_features_univariate) < 2) {
  ...
  if(any(is.na(x_lasso)) | any(is.na(y_lasso))) {
    ...
    if(!is.null(cv_fit)) {
      ...
      # 找到选择10个变量的lambda索引
      lambda_idx <- which(num_features_selected == 10)[1]
      
      if(!is.na(lambda_idx)) {
        # 找到选择10个变量的lambda
        lambda_10feat <- lasso_full$lambda[lambda_idx]
        lasso_model_10feat <- glmnet(
          x = x_lasso, 
          y = y_lasso, 
          alpha = 1, 
          lambda = lambda_10feat,
          family = "binomial"
        )
        
        lasso_coef_matrix <- as.matrix(coef(lasso_model_10feat))
        ...
        cat("LASSO selected 10 features at lambda =", round(lambda_10feat, 6), "\n")
      } else {
        # 如果没有正好选择10个变量的lambda，找到最接近的
        cat("No lambda selects exactly 10 features, finding closest match...\n")
        # 找到选择变量数量最接近10的lambda
        diff_features <- abs(num_features_selected - 10)
        ...
        cat("LASSO selected", num_selected, "features (closest to 10) at lambda =", round(lambda_closest, 6), "\n")
      }
      
      # 如果LASSO没有选择任何特征，使用单变量分析结果
      if(length(selected_features_lasso) == 0) {
        cat("Warning: LASSO did not select any features, using top 10 univariate features...\n")
        # 取单变量分析中P值最小的10个特征
        if(length(selected_features_univariate) >= 10) {
          selected_features_lasso <- head(selected_features_univariate, 10)
        } else {
      ...
      cat("Using top 10 univariate features due to LASSO failure\n")
      # 取单变量分析中P值最小的10个特征
      if(length(selected_features_univariate) >= 10) {
        final_features <- head(selected_features_univariate, 10)
      } else {
        final_features <- selected_features_univariate
      }
    }
...
# 确保最终选择10个特征（如果可用）
if(length(final_features) > 10) {
  cat("Selected more than 10 features, keeping top 10 by LASSO coefficients...\n")
  # 重新运行LASSO获取系数绝对值
  ...
  # 使用一个较小的lambda以保留更多特征，然后选择系数绝对值最大的10个
  ...
  top10_idx <- order(coef_abs, decreasing = TRUE)[1:min(10, length(coef_abs))]
  final_features <- rownames(lasso_coef_no_intercept)[top10_idx]
} else if(length(final_features) < 10 && length(selected_features_univariate) >= 10) {
  cat("Selected less than 10 features, supplementing with top univariate features...\n")
  # 补充单变量分析中P值最小的特征直到有10个
  top_univariate <- head(selected_features_univariate, 10)
  final_features <- unique(c(final_features, top_univariate))[1:10]
}

cat("Final selected features (target: 10):", length(final_features), "\n")
...
if(length(selected_features_univariate) >= 10) {
  ...
  if(exists("lambda_10feat")) {
    abline(v = log(lambda_10feat), col = "red", lty = 2, lwd = 2)
    legend("topright", legend = paste("10 features (λ =", round(lambda_10feat, 4), ")"), 
...
cat("After LASSO selection (target: 10):", length(final_features), "\n")

cat("\nTop 10 Features by Importance:\n")
for(i in 1:min(10, nrow(top_20_features))) {
